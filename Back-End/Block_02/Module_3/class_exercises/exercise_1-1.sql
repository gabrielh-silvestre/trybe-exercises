...
/* 2 */ SELECT first_name FROM sakila.customer
WHERE active = 0 AND store_id = 2 AND first_name <> 'KENNETH';

/* 3 */ SELECT title, `description`, release_year, replacement_cost
FROM sakila.film
WHERE replacement_cost >= 18.00 AND rating <> 'NC-17'
ORDER BY replacement_cost DESC, title
LIMIT 100;

/* 4 */ SELECT COUNT(*) FROM sakila.customer
WHERE active = 1 AND store_id = 1;

/* 5 */ SELECT * FROM sakila.customer
WHERE active = 1 AND store_id = 1;
